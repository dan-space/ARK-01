...
function unlock(){if(unlocked)return;try{context=window.AC||new(window.AudioContext||window.webkitAudioContext)();context.resume();unlocked=true;const s=reader?.();if(s?.legacy&&s.legacy.muted===enabled)s.legacy.toggle();update()}catch(e){}}
