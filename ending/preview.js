...
function selectVoice(who){const voices=synth?synth.getVoices():[],en=voices.filter(v=>/^en(-|_)/i.test(v.lang));const primary=en.find(v=>/zira|samantha|google|microsoft/i.test(v.name))||en[0];if(who==='elena')return en.find(v=>v!==primary&&/samantha|victoria|karen|zira|aria|jenny|susan|female/i.test(v.name))||en.find(v=>v!==primary)||primary;return primary}
...
function run(i){clearPlayback();if(i>=turns.length)return finish();index=i;const token=generation;let started=false,done=false;const commit=()=>{if(token!==generation)return;started=true;showTurn(i);duckFX(true);$('status').textContent='Speaking · picture and dialogue synchronized';};$('next').hidden=false;$('skip').hidden=false;$('play').hidden=true;$('replay').hidden=true;
 ...
 $('status').textContent='Preparing the next voice…';const u=new SpeechSynthesisUtterance(turns[i][2]);active=u;u.lang='en-US';u.voice=selectVoice(turns[i][1])||null;u.rate=turns[i][3];u.pitch=turns[i][1]==='elena'?1.08:turns[i][1]==='alex'?.88:.98;u.volume=1;
...
function finish(){clearPlayback();stopFX();victoryMusic();currentScene='';$('film').dataset.scene='home';$('scene').src='../assets/images/ark01/earth-second-chance.png';$('stage').textContent='OLD EARTH · A SECOND CHANCE';$('title').textContent='HOME IS WORTH SAVING';$('subtitle').textContent='Humanity returns to restore the Earth it left behind.';$('speaker').textContent='ARK-01 · RETURN COURSE CONFIRMED';$('line').textContent='The search ends where our responsibility began. The passengers return, not to escape what happened, but to rebuild a home together.';$('status').textContent='Home again. A new beginning.';$('summary').hidden=false;$('next').hidden=true;$('skip').hidden=true;$('replay').hidden=false;$('progress').textContent='END OF PREVIEW'}
async function start(){$('summary').hidden=true;clearPlayback();stopFX();currentScene='';try{context=context||new(window.AudioContext||window.webkitAudioContext)();await context.resume()}catch(e){}run(0)}
...
window.addEventListener('message',e=>{if(e.source!==window.parent||e.data?.channel!=='ark-audio-settings')return;effectLevel=Math.max(0,Math.min(1,Number(e.data.volume??1)));voiceOn=e.data.voice!==false;sfxOn=!e.data.muted&&e.data.volume>0;$('audio').textContent='VOICE · '+(voiceOn?'ON':'OFF');$('effects').textContent='SOUND · '+(sfxOn?'ON':'OFF')});
